...
입주민 계정 관리 기능 - 회원가입 - 아이디, 비밀번호, 연락처, 입주 아파트명, 동, 호수 등 개인 정보 입력과 아파트 정보를 입력하여 회원가입 요청 - 아파트 관리자가 승인 완료 시 회원가입이 완료되며 승인 대기 중이거나 거절된 상태이면 로그인이 불가능

입주민 관리 기능(가입한 입주민 + 미가입한 입주민 관리)
